...
export async function extendLUT() {
  ...
  const response = await prompts([
    {
      type: "text",
      name: "vanityPrompt",
      message: "Do you want to import a custom vanity address? (y/n):",
    },
    {
      type: "number",
      float: true,
  ...
  const { vanityPrompt } = response;
  const jitoTipAmt = +response.jitoTipAmt * LAMPORTS_PER_SOL;
  vanityPK = response.vanityPK;

  if (vanityPrompt === "y") {
    const respPk = await prompts({
      type: "text",
      name: "vanityPK",
      message: "Enter the private key of the vanity address (bs58)",
    });

    vanityPK = respPk.vanityPK;
  }
...
export async function createLUT() {
  // -------- step 1: ask nessesary questions for LUT build --------
  console.log("step 1: ask nessesary questions for LUT build");
  ...
  console.log("Jito tip in Sol:", answer.jitoTipAmt);
  const jitoTipAmt = answer.jitoTipAmt * LAMPORTS_PER_SOL;
  console.log("Jito tip in lamports:", jitoTipAmt);
  ...
  console.log("step 2: create a new LUT every time there is a new launch");
  ...
  console.log("write to keyInfo.json");
  // Write updated content back to poolInfo.json
  fs.writeFileSync(keyInfoPath, JSON.stringify(poolInfo, null, 2));

  console.log("push createLUT to bundle");
  // Push to bundle
  bundledTxns.push(createLUT);

  // -------- step 3: SEND BUNDLE --------
  console.log("step 3: SEND BUNDLE");

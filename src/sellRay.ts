...
export async function sellXPercentageRAY() {
  ...
  const PayerTokenATA = await spl.getAssociatedTokenAddress(
  ...
  for (let chunkIndex = 0; chunkIndex < chunkedKeypairs.length; chunkIndex++) {
    ...
    for (let keypair of chunk) {
      ...
      sellTotalAmount += transferAmount; // Keep track to sell at the end
      console.log(
        `Sending ${transferAmount / 1e6} from ${keypair.publicKey.toString()}.`
      );
  ...
  const PayerwSolATA = await spl.getAssociatedTokenAddress(
    spl.NATIVE_MINT,
    payer.publicKey
  );
